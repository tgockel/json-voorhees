...
protected:
    virtual std::expected<T, ast_node_type> create(extraction_context& context, reader& from) const override
    {
        ...
        if constexpr (detail::is_expected_v<decltype(output)>)
        {
            return output;
        }
        else
        {
            return std::expected<T, ast_node_type>{ std::move(output) };
        }
    }
...
auto make_adapter(FExtract extract, FToJson to_json_)
    -> function_adapter<detail::expected_value_or_self_t<decltype(extract(std::declval<extraction_context&>(),
                                                                          std::declval<reader&>()))>,
                        FExtract,
                        FToJson
                       >
{
    return function_adapter<detail::expected_value_or_self_t<decltype(extract(std::declval<extraction_context&>(),
                                                                              std::declval<reader&>()))>,
                            FExtract,
                            FToJson
                           >
...
auto make_adapter(FExtract extract, FToJson to_json_)
    -> function_adapter<detail::expected_value_or_self_t<decltype(extract(std::declval<reader&>()))>,
                        FExtract,
                        FToJson
                       >
{
    return function_adapter<detail::expected_value_or_self_t<decltype(extract(std::declval<reader&>()))>,
                            FExtract,
                            FToJson
                           >
            (std::move(extract), std::move(to_json_));
