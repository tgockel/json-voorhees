                                    ...
                                    )
{
    static auto instance =
        make_adapter([] (extraction_context& cxt, reader& from) -> std::expected<T, ast_node_type>
                     {
                         return cxt.current_as<ast_node::integer>(from)
                            .transform([](const ast_node::integer& x)
                            {
...
static formats create_default_formats()
{
    ...
    static auto string_extractor =
        make_adapter([] (extraction_context& cxt, reader& from) -> std::expected<std::string, ast_node_type>
                     {
                        if (auto res = cxt.expect(from, { ast_node_type::string_canonical,
                                                          ast_node_type::string_escaped });
                            !res)
                        {
        ...
        make_adapter([] (extraction_context& cxt, reader& from)
                     {
                         return cxt.current_as<ast_node::string_canonical>(from)
                            .transform([](const auto& x) { return x.value(); });
                     },
    ...
    static auto bool_extractor =
        make_adapter([] (extraction_context& cxt, reader& from) -> std::expected<bool, ast_node_type>
                     {
                         if (auto res = cxt.expect(from, { ast_node_type::literal_false,
                                                           ast_node_type::literal_true });
                             !res)
                         {
                             return std::unexpected(res.error());
                         }
    ...
    static auto double_extractor =
        make_adapter([] (extraction_context& cxt, reader& from) -> std::expected<double, ast_node_type>
                     {
    ...
    static auto float_extractor =
        make_adapter([] (extraction_context& cxt, reader& from) -> std::expected<float, ast_node_type>
                     {
                         return cxt.current_as<ast_node::decimal>(from)
                            .transform([] (const auto& x) { return float(x.value()); });
                     },
...
{
    static auto instance =
        make_extractor([] (extraction_context& cxt, reader& from) -> std::expected<T, ast_node_type>
                       {
                           ...
                           case ast_node_type::integer:
                               return detail::clamp_cast<T>(current.as<ast_node::integer>().value());
                           case ast_node_type::string_canonical:
                           case ast_node_type::string_escaped:
                               return detail::clamp_cast<T>(coerce_integer(value(coerce_string(from))));
                           default:
...
static formats create_coerce_formats()
{
    ...
    static auto string_extractor =
        make_extractor([](reader& from) -> std::expected<std::string, ast_node_type>
                       {
                           return coerce_string(from);
                       });
    fmt.register_extractor(&string_extractor);

    static auto bool_extractor =
        make_extractor([] (reader& from) -> std::expected<bool, ast_node_type>
                       {
                           return coerce_boolean(from);
                       });
    ...
    static auto double_extractor =
        make_extractor([] (reader& from) -> std::expected<double, ast_node_type>
                       {
                           return coerce_decimal(from);
                       });
    fmt.register_extractor(&double_extractor);
    static auto float_extractor =
        make_extractor([] (reader& from) -> std::expected<float, ast_node_type>
                       {
                           return float(coerce_decimal(from));
                       });
